...
class LeaveRepository @Inject constructor(
    ...
    fun hasDrivePermission(): Boolean = driveSyncManager.hasDrivePermission()
    ...
    suspend fun getLeaveBalance(kgid: String): LeaveBalance? {
        ...
        return try {
            ...
            if (localBalance != null) {
                ...
                val profileEmail = sessionManager.userEmail.first().ifBlank { null }
                val result = driveSyncManager.downloadBackup(profileEmail)
                if (result is GoogleDriveSyncManager.SyncResult.Success && result.data?.balance?.kgid == kgid) {
                    ...
                    Log.d(TAG, "No backup found, creating new balance")
    ...
    suspend fun updateHplManualBalance(balance: LeaveBalance) {
        try {
            leaveDao.insertBalance(balance.toEntity())
            triggerSync(balance.kgid)
            Log.d(TAG, "HPL manual balance updated locally")
        } catch (e: Exception) {
    ...
    suspend fun manualBackup(kgid: String): GoogleDriveSyncManager.SyncResult = withContext(Dispatchers.IO) {
        try {
            ...
            val profileEmail = sessionManager.userEmail.first().ifBlank { null }
            val result = driveSyncManager.uploadBackup(balance, entries, logs, profileEmail)
            if (result is GoogleDriveSyncManager.SyncResult.Success) {
            ...
            GoogleDriveSyncManager.SyncResult.Error(e)
        }
    }

    suspend fun manualRestore(kgid: String): GoogleDriveSyncManager.SyncResult = withContext(Dispatchers.IO) {
        try {
            ...
            if (result is GoogleDriveSyncManager.SyncResult.Success && result.data?.balance?.kgid == kgid) {
                ...
                result
            } else if (result is GoogleDriveSyncManager.SyncResult.Success) {
                Log.w(TAG, "Manual restore failed: KGID mismatch in backup")
                GoogleDriveSyncManager.SyncResult.NoBackupFound
            } else {
                Log.w(TAG, "Manual restore failed: $result")
                result
            }
        } catch (e: Exception) {
    ...
    private suspend fun triggerSync(kgid: String, retryCount: Int = 1) {
        withContext(Dispatchers.IO) {
            try {
                ...
                val profileEmail = sessionManager.userEmail.first().ifBlank { null }
                val result = driveSyncManager.uploadBackup(balance, entries, logs, profileEmail)
                if (result !is GoogleDriveSyncManager.SyncResult.Success && retryCount > 0) {
                    Log.w(TAG, "Background sync failed for $kgid, retrying ($retryCount left)... Status: $result")
