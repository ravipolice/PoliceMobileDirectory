    ...
    fun calculateEmployeeScore(
        ...
        return when (filter) {
            SearchFilter.ALL -> {
                 ...
                 val unitScore = calculateFieldScore(employee.unit, queryLower) * 0.5
                 val metalScore = calculateFieldScore(employee.metalNumber, queryLower) * 0.6
                 maxOf(nameScore, idScore, mobileScore, rankScore, stationScore, unitScore, metalScore)
            }
    ...
    fun calculateOfficerScore(
        ...
        return when (filter.lowercase()) {
            ...
            "district" -> calculateFieldScore(officer.district, queryLower) * FieldWeights.DISTRICT_EXACT
                ...
                val stationScore = calculateFieldScore(officer.station, queryLower) * FieldWeights.STATION_EXACT
                val districtScore = calculateFieldScore(officer.district, queryLower) * FieldWeights.DISTRICT_EXACT
                maxOf(nameScore, idScore, mobileScore, rankScore, stationScore, districtScore)
            }
        }
    }
    ...
    private fun getMatchedFields(employee: Employee, queryLower: String, filter: SearchFilter): List<String> {
        ...
        when (filter) {
            SearchFilter.ALL -> {
                ...
                if (employee.metalNumber?.lowercase()?.contains(queryLower) == true) matched.add("metalNumber")
                if (employee.bloodGroup?.lowercase()?.contains(queryLower) == true) matched.add("bloodGroup")
            }
    ...
    private fun getMatchedOfficerFields(officer: Officer, queryLower: String, filter: String): List<String> {
        ...
        when (filter.lowercase()) {
            ...
            "district" -> if (officer.district?.lowercase()?.contains(queryLower) == true) matched.add("district")
                ...
                if (officer.district?.lowercase()?.contains(queryLower) == true) matched.add("district")
            }
        }
